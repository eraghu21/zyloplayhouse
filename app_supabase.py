"""
Membership ERP - Supabase-backed Streamlit app (Full CRM / Business Suite)
Features:
- Supabase PostgreSQL backend (SERVICE_ROLE key required for admin operations)
- Members, Plans, Member Plans, Visits, Payments/Invoices, Users (staff)
- Staff login (email + password stored hashed in Supabase users table)
- Add/Edit/Delete members, assign plans, record visits
- Create invoices/payments and track status
- Notifications placeholder (can be wired to email/SMS)
- Export to Excel, reports dashboard
- Uses st.toast for auto-closing alerts and st.rerun()
- SECURITY: don't commit SERVICE_ROLE key to public repos; use Streamlit secrets
"""
...
# Read Supabase credentials from environment (set as Streamlit secrets)
SUPABASE_URL = os.environ.get("SUPABASE_URL")
SUPABASE_SERVICE_ROLE_KEY = os.environ.get("SUPABASE_SERVICE_ROLE_KEY")

if not SUPABASE_URL or not SUPABASE_SERVICE_ROLE_KEY:
    st.error("Supabase credentials not found. Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY in environment/secrets.")
    st.stop()
...
pages = ["Login","Dashboard","Members","Plans","Member Plans","Visits","Invoices","Reports","Export","Settings"]
...
if page == "Dashboard":
    ...
    if st.button("Add quick member sample"):
        add_member("Parent X","9999999999","Child X", datetime.now().date().isoformat())
...
if page == "Export":
    ...
    if st.button("Export all to Excel"):
        ...
        # ensure membership_no preserved
